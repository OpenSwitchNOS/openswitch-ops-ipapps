...
show_dhcp_relay_config (void)
{
    ...
    {
        dhcp_relay_status = "enabled";
    }
    else
    {
        if (!strcmp(dhcp_relay_status, FALSE))
            dhcp_relay_status = "enabled";
        else
            dhcp_relay_status = "disabled";
    }
    ...
    {
        vty_out(vty, "%s%s", "enabled", VTY_NEWLINE);
        return CMD_SUCCESS;
    }
}
...
{
    udpfwd_feature udpfwdServer;
    config_type type = DHCP_RELAY;
    memset(&udpfwdServer, 0, sizeof(udpfwd_feature));

    /* Validate the input parameters. */
    if (decode_server_param(&udpfwdServer, argv, type))
    {
      ...
      SUBIFNAME_STR)
{
    ...
    if (argv[0])
    {
        return show_ip_helper_address_config(argv[0], type);
    }
    else
    {
        return show_ip_helper_address_config(NULL, type);
    }
}
