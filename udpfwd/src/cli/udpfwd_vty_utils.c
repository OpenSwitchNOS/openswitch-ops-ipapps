...
udpfwd_globalconfig (const char *disable, udpfwd_feature type)
{
    ...
    if (strcmp(disable, "true") == 0)
    {
        smap_add(&smap_status_value, key, disable);
    }
    else
    {
        smap_remove(&smap_status_value, key);
    }
                                         ...
                                         const char *vrf_name)
{
    ...
    OVSREC_DHCP_RELAY_FOR_EACH (row_serv, idl)
    {
        ...
        {
            if (strcmp(row_serv->vrf->name, vrf_name) == 0)
            {
                return row_serv;
            }
        }
...
-----------------------------------------------------------------------------*/
bool
is_helper_address_count_max(const char *portname)
{
    const struct ovsrec_dhcp_relay *row_serv = NULL;
    size_t entries = 1;
...
udpfwd_serverupdate (const struct
                     ...
                     int8_t action, udpfwd_server *udpfwdServ)
{
    char **servers;
...
udpfwd_serverconfig (udpfwd_server *udpfwdServ, bool set)
{
    ...
    if (row_serv)
    {
        isAddrMatch = find_udpfwd_server_ip(row_serv->ipv4_ucast_server,
                                            ...
                                            udpfwdServ, DHCP_RELAY);
    }

    if (set)
    {
        isMaxEntries = is_helper_address_count_max((char*)vty->index);
        if (isMaxEntries)
        {
