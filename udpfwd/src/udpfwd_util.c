...
uint16_t get_udp_csum(struct ip *iph, struct udphdr *udph)
{
    struct csum_construct *udp_csum_construct;
    struct pseudoheader *ph;
    int payload_len = 0;

    udp_csum_construct = &udpfwd_ctrl_cb_p->udp_csum_construct;
    ph = &udp_csum_construct->pshd;

    ph->src_addr = iph->ip_src.s_addr;
    ph->dst_addr = iph->ip_dst.s_addr;
    ph->padding = 0;
    ph->proto = iph->ip_p;
    ph->length = htons(udph->uh_ulen);

    udp_csum_construct->udph = *udph;
    payload_len = udph->uh_ulen - sizeof(struct udphdr);

    memcpy(udp_csum_construct->payload, (char *)udph + sizeof(struct udphdr),
           payload_len);

    return in_cksum((uint16_t*) udp_csum_construct,
                    (sizeof(struct pseudoheader) + udph->uh_ulen), 0);
}
