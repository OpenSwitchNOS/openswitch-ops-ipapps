...
/* FIXME: temporary main function added to avoid zuul failure.*/
int main()
{
    return 0;
}
 ...
 *                 otherwise -1.
 */
int create_udp_bcast_socket(void)
{
    int sockfd = -1;
 ...
 * Return        : none
 */
void udpfwd_ctrl(void *payload, int size,
...
void * udp_packet_recv(void *args)
{
    ...
    struct iovec iov;
    int size, sockfd = -1;
    unsigned int ifinput = -1;
    char ifName[IFNAME_LEN];

    udpfwd_ctrl_cb_p->buff = (char *) calloc(RECV_BUFFER_SIZE, sizeof(char));
    ...
    iov.iov_base = (void *) udpfwd_ctrl_cb_p->buff; /* buffer to store udp packet payload */
