...
/*
 * Function: add_dhcp_relay_option82_field
 * Responsibility: Add option 82 field in dhcp packet.
 * Parameters: pkt - received DHCP packet
 *             length - length of packet
 *             sp - pointer to iterate through the packet
 *             max_msg_size -  maximum msg size
 *             remote_id - stores remote_id
...
void add_dhcp_relay_option82_field(void *pkt, uint8_t *sp,
                                uint32_t *length, uint32_t ifIndex,
                                char * ifName, uint16_t max_msg_size,
                                DHCP_RELAY_OPTION82_REMOTE_ID remote_id,
...
bool process_dhcp_relay_option82_message(void *pkt, DHCP_OPTION_82_OPTIONS *pkt_info,
                               uint32_t ifIndex, char *ifName, int32_t *new_size)
{
    struct ip *iph = NULL;       /* pointer to IP header */
    ...
    else if (dhcp->op == BOOTREQUEST)
    {
        ...
        * beginning of the pad.  */
        ...
        add_dhcp_relay_option82_field(pkt, sp, &length, ifIndex, ifName,
                                    max_msg_size, remote_id, pkt_info);
   ...
   iph->ip_len = htons(length + UDPHDR_LENGTH + ((char *)iph + (iph->ip_hl * 4)));
