                                  ...
                                  struct ds *ds)
{
    ...
    for(iter = 0; iter < intfNode->addrCount; iter++)
    {
        ...
        ds_put_format(ds, "%s,%d,egress %s ", server->ipv6_address, server->ref_count, server->egressIfName);
    }
    return;
}
...
bool dhcpv6_relay_init()
{
    ...
    unixctl_command_register("dhcpv6_relay/dump", "", 0, 2,
