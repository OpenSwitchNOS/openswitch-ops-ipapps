...
void dhcpv6r_server_config_update(void)
{
    ...
    /*FIXME : below changes needs to be tested.*/
    /* For now, primary ipv6 address is being used as source interface address */
    ...
    if (OVSREC_IDL_IS_ROW_MODIFIED(port_row, idl_seqno))
    {
        if(OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_port_col_ip6_address,
            ...
            if (NULL != node) {
                ...
                intfNode->ip6_address = port_row->ip6_address;
            }
        }
    }
    return;
...
static void dhcpv6r_interface_dump(struct shash_node *node,
                                  struct ds *ds)
{
    /*FIXME : Need to add statistics to unixctl output */
