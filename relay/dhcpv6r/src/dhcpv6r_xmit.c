                           ...
                           uint32_t srcRecv_ifindex)
{
    ...
    {
        VLOG_ERR("Failed to read input interface : %d", srcRecv_ifindex);
        return false;
    }
    ...
    if (!(dhcpv6r_send((char *) pktPtr, pktLen, true, srcRecv_ifindex,
                        NULL, true, intfNode, srcAddr.sin6_scope_id)))
    {
...
bool dhcpv6r_relay_to_client (dhcpv6r_basePkt_t *pktPtr, int pktLen)
{
    ...
    if ((-1 == clientData.clientIntf) ||
        ...
        VLOG_ERR("Failed to read input interface : %d", clientData.clientIntf);
        return false;
    }
    ...
    if (!(dhcpv6r_send(sbuf, txLen, false, clientData.clientIntf,
         &(clientData.clientAddress), sendToRelay, NULL, 0)))
    {
                  ...
                  uint32_t scopeId)
{
    ...
    else
    {
        ...
        for(iter = 0; iter < intfNode->addrCount; iter++) {
            ...
            {
                /*FIXME : Need to test this condition of code */
