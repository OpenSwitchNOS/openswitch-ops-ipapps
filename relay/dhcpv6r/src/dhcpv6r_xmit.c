...
/* FIXME : End to End testing needs to be done once sendto problem is fixed,
 * only client to server one way path works when dest port set to 0
 * Statistics at various places needs to be updated
 */
 ...
 *                 false - failure
 */
bool dhcpv6r_relay_to_server (char *recvPkt, int rxPktLen,
                           struct sockaddr_in6 srcAddr, uint32_t srcRecv_ifindex)
{
    uint32_t pktLen = 0;
    ...
    dhcpv6r_basePkt_t *optPtr = NULL, *rxPktPtr = NULL;
    ...
    /* 8 = size of UDP header , Extract UDP header first*/
    udph = (struct udphdr *) (recvPkt);
    udph->uh_dport = htons(DHCPV6_RELAY_PORT);
    udph->uh_sport = htons(DHCPV6_RELAY_PORT);
    udph->check = 0;
    udph->len = htons(sizeof(struct udphdr));

    rxPktPtr = (dhcpv6r_basePkt_t *)((char*)recvPkt + UDP_HEADER);
    ...
    if (pktFromClient)
    {
        ...
        */
        ...
        if (getIpv6Address(intfNode->ip6_address, &ss))
        {
            ...
            inet_ntop(AF_INET6, (void *)&pkt.linkAddress, ipv6Str, INET6_ADDRSTRLEN);
        }
    }
    else
    {
    ...
    inet_ntop(AF_INET6, (void *)&pkt.peerAddress, ipv6Str, INET6_ADDRSTRLEN);

    memcpy(pktPtr, udph, sizeof(struct udphdr));
    pktPtr = pktPtr + UDP_HEADER;
    pktLen = pktLen + sizeof(struct udphdr);

    memcpy(pktPtr, &pkt, sizeof(pkt));
    pktLen = pktLen + sizeof(pkt);

    optPtr = (pktPtr + 1);

    dhcpv6_relay_ctrl_cb_p->dhcpv6_relay_option79_enable = true;
    ...
    if (!(dhcpv6r_send((char *) pktPtr, pktLen, true, srcRecv_ifindex, NULL, true, intfNode, srcAddr)))
    {
 ...
 *                 false - failure
 */
bool dhcpv6r_relay_to_client (char *recvPtr, int pktLen,
                           struct sockaddr_in6 srcAddr)
{
    ...
    memset(sbuf, 0, sizeof(sbuf));
    pktPtr = (dhcpv6r_basePkt_t*)(recvPtr + UDP_HEADER);
    /* FIXME: Need to fill udp header info */
                  ...
                  struct sockaddr_in6 srcAddr)
{
    ...
    else
    {
        ...
        for(iter = 0; iter < intfNode->addrCount; iter++) {
            ...
            memset(&dst, 0, sizeof(dst));
            dst.sin6_family = PF_INET6;
            dst.sin6_port = 0; /*FIXME : need to set value to 547 */

            inet_pton(PF_INET6, server->ipv6_address, &(dst.sin6_addr));

            if (IN6_IS_ADDR_MULTICAST(&(dst.sin6_addr)))
            {
               /*FIXME : test this code by configuring server ip as mcast */
            ...
            {
                /*FIXME : test this code by configuring server ip as link local */
