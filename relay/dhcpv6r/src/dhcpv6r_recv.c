                                        ...
                                        bool joinFlag)
{
    ...
    if (joinFlag)
    {
        if (setsockopt(dhcpv6_relay_ctrl_cb_p->dhcpv6r_recvSock, IPPROTO_IPV6,
...
void * dhcpv6r_recv(void *args)
{
   ...
   struct udphdr *udph;
    ...
    assert(dhcpv6_relay_ctrl_cb_p->dhcpv6r_recvSock);

    while (true)
    {
        size = recvmsg(dhcpv6_relay_ctrl_cb_p->dhcpv6r_recvSock, &mhdr, 0);
        if (size < 0)
        {
        ...
        if (ntohs(udph->dest) == DHCPV6_RELAY_PORT)
        {
            ...
            if (dhcpv6r_is_msg_from_client(dhcpv6_msgType))
            {
                ...
                if ((-1 == recv_ifindex) ||
                    ...
                    VLOG_ERR("Failed to read input interface : %d", recv_ifindex);
                    return NULL;
                }
                if (!(dhcpv6r_relay_to_server((dhcpv6r_basePkt_t*)((char*)iov.iov_base + UDP_HEADER),
                                                (size-UDP_HEADER), from, recv_ifindex)))
                {
            ...
            else
            {
                ...
                if (!(dhcpv6r_relay_to_client((dhcpv6r_basePkt_t*)((char*)iov.iov_base + UDP_HEADER),
                                                (size-UDP_HEADER))))
                {
                    VLOG_ERR("Packet received on interface: %s failed"
                              " to relay to client", ifName);
                }
            }
        }
    }
