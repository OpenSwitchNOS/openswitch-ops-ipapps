 ...
 * Responsiblity : To join or leave the All_Dhcp_Relay_Agents_And_Servers multicast
...
void * dhcpv6r_recv(void *args)
{
   ...
   while (true)
   {
        ...
        /* Make sure that the packet recd. is not IPv4 mapped IPv6 address. */
        ...
        if (dhcpv6r_isMsgFromClient(dhcpv6_msgType))
        {
            ...
            if (!(dhcpv6r_relayToServer
            ((dhcpv6r_basePkt_t*)iov.iov_base, size, from, recv_ifindex, recv_ifindex)))
            {
                VLOG_ERR("Packet received on interface: %s failed to relay to server", ifName);
            }
        }
    }
}
