    ...
    SHASH_FOR_EACH_SAFE(node, next, &dhcpv6_relay_ctrl_cb_p->intfHashTable) {
        ...
        if (false == found) {
            ...
            for (iter = 0; iter < intf->addrCount; iter++) {
                dhcpv6_relay_remove_address(intf, serverArray[iter]->ipv6_address,
...
/*
 * Function      : dhcpv6_relay_get_ucast_entries_removed
...
void dhcpv6_relay_get_ucast_entries_removed(DHCPV6_RELAY_INTERFACE_NODE_T *intfNode,
                        const struct ovsrec_dhcp_relay *rec)
{
    ...
    for (iter = 0; iter < intfNode->addrCount; iter++) {
        ...
        for (iter1 = 0; iter1 < rec->n_ipv6_ucast_server; iter1++) {
            if (0 == strncmp(ipv6_address, rec->ipv6_ucast_server[iter],
                strlen(ipv6_address))) {
                found = true;
                break;
            }
        ...
        if (false == found) {
            if(!dhcpv6_relay_remove_address(intfNode, ipv6_address, "default"))
...
/*
 * Function      : dhcpv6_relay_get_ucast_entries_added
...
void dhcpv6_relay_get_ucast_entries_added(DHCPV6_RELAY_INTERFACE_NODE_T *intfNode,
                        const struct ovsrec_dhcp_relay *rec)
{
    DHCPV6_RELAY_SERVER_T *arrayPtr = NULL;
    int iter = 0;
    bool found;

    for (iter = 0; iter < rec->n_ipv6_ucast_server; iter++) {
        found = false;
        arrayPtr = dhcpv6_relay_get_server_entry(rec->ipv6_ucast_server[iter], "default");
        if (arrayPtr != NULL)
        {
            found = true;
            break;
        ...
        if (false == found) {
            /* for ucast ipv6 address entries, storing egressName as default */
            if (!dhcpv6_relay_store_address(intfNode, rec->ipv6_ucast_server[iter], "default"))
...
/*
 * Function      : dhcpv6_relay_get_mcast_entries_added
 * Responsiblity : Check for mcast entries added in dhcp relay table
 * Parameters    : rec - DHCP-Relay OVSDB table record
 *                 intfNode - Interface entry
 * Return        : none
 */
void dhcpv6_relay_get_mcast_entries_added(DHCPV6_RELAY_INTERFACE_NODE_T *intfNode,
                        const struct ovsrec_dhcp_relay *rec)
{
    char *outIfName = NULL;
    DHCPV6_RELAY_SERVER_T *arrayPtr = NULL;
    ...
    SMAP_FOR_EACH(smap_node, &rec->ipv6_mcast_server) {
        ...
        if (smap_node->key)
        {
            ...
            while(outIfName != NULL) {
                arrayPtr = dhcpv6_relay_get_server_entry(smap_node->key, outIfName);
                if (arrayPtr != NULL)
                {
                    found = true;
                    break;
                ...
                if (false == found) {
                    if (!dhcpv6_relay_store_address(intfNode, smap_node->key, outIfName))
...
/*
 * Function      : dhcpv6_relay_get_mcast_entries_removed
 ...
 * Return        : none
 */
void dhcpv6_relay_get_mcast_entries_removed(DHCPV6_RELAY_INTERFACE_NODE_T *intfNode,
                        const struct ovsrec_dhcp_relay *rec)
{
    ...
    for (iter = 0; iter < intfNode->addrCount; iter++) {
        ...
        for (iter1 = 0; iter1 < size; iter1++) {
            if ((0 == strcmp(ipv6_address, tempServers[iter1].ipv6_address)) &&
                (0 == strcmp(egressIfName, tempServers[iter1].egressIfName))) {
                found = true;
                break;
            }
        }
        if (false == found) {
            if(!dhcpv6_relay_remove_address(intfNode, ipv6_address, egressIfName))
              ...
              const struct ovsrec_dhcp_relay *rec, uint32_t idl_seqno)
{
    ...
    if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_dhcp_relay_col_ipv6_ucast_server,
                               idl_seqno)) {
        dhcpv6_relay_get_ucast_entries_added(intfNode, rec);
        dhcpv6_relay_get_ucast_entries_removed(intfNode, rec);
    }

#if 0 /* Code changes to be enabled once schema is pushed */
    if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_dhcp_relay_col_ipv6_mcast_server,
                           idl_seqno)) {
        dhcpv6_relay_get_mcast_entries_added(intfNode, rec);
        dhcpv6_relay_get_mcast_entries_removed(intfNode, rec);
    }
#endif
    return;
}
