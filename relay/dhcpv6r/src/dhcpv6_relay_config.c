...
/*
 * Function      : compare
 * Responsiblity : compare ipv6 address and ifNames
 * Parameters    : ip1, ip2 - ipv6 addresses
 *                 ifName1, ifName2 - egress interface
 * Return        : true - success
 *                 false - failure
 */
bool compare(char *ip1, char*ip2, char*ifName1, char*ifName2)
{
    COMPARE_IP(ip1, ip2)
    {
                                        ...
                                        char* egressIfName)
{
    ...
    CMAP_FOR_EACH_WITH_HASH(serverIP, cmap_node, hash_key,
        ...
        if (compare(ipv6_address, serverIP->ipv6_address,
                                ...
                                int *deleted_index)
{
    ...
    {
        if (compare(serverArray[index]->ipv6_address, ipv6_address,
                        ...
                        const struct ovsrec_dhcp_relay *rec)
{
    ...
    for (iter = 0; iter < intfNode->addrCount; iter++) {
        ...
        for (iter1 = 0; iter1 < rec->n_ipv6_ucast_server; iter1++) {
            COMPARE_IP(ipv6_address, rec->ipv6_ucast_server[iter])
            {
                found = true;
                break;
            }
                        ...
                        const struct ovsrec_dhcp_relay *rec)
{
    ...
    for (iter = 0; iter < rec->n_ipv6_ucast_server; iter++) {
        found = false;
        arrayPtr = dhcpv6r_get_server_entry(rec->ipv6_ucast_server[iter], NULL);
        if (arrayPtr != NULL)
        {
            found = true;
            break;
    ...
    SMAP_FOR_EACH(smap_node, &rec->ipv6_mcast_server) {
        ...
        if (smap_node->key)
        {
            ...
            while(outIfName != NULL) {
                arrayPtr = dhcpv6r_get_server_entry(smap_node->key, outIfName);
                if (arrayPtr != NULL)
                {
                    found = true;
                    break;
                        ...
                        const struct ovsrec_dhcp_relay *rec)
{
    ...
    for (iter = 0; iter < intfNode->addrCount; iter++) {
        ...
        for (iter1 = 0; iter1 < size; iter1++) {
            if (compare(ipv6_address, tempServers[iter1].ipv6_address,
