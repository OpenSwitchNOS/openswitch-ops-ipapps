...
#include "udpfwd/include/udpfwd.h"
#include "dhcpv6_relay/include/dhcpv6r.h"
...
extern void udpfwd_unixctl_dump(struct unixctl_conn *conn, int argc OVS_UNUSED,
 ...
 *                 false - on failure
 */
bool relay_init(const char *remote)
{
    ...
    /* Initialize udpfwd module data structures */
    if (true != udpfwd_module_init())
    {
        VLOG_FATAL("udpfwd module failed to initialize");
        return false;
    }

    /* Initialize dhcpv6r module data structures */
    if (true != dhcpv6r_module_init())
    {
        VLOG_FATAL("dhcpv6r module failed to initialize");
        return false;
    }
