...
{
    if (strncmp(protocol_name, TFTP, strlen (protocol_name)) == 0) {
        return TFTP_PROTOCOL;
    } else if (strncmp(protocol_name, TACACS, strlen (protocol_name)) == 0) {
        return TACACS_PROTOCOL;
    } else if (strncmp(protocol_name, RADIUS, strlen (protocol_name)) == 0) {
      ...
      RADIUS_STRING)
{
    if (argv[0])
    {
        return show_source_interface_selection(get_protocol_by_name((char*)argv[0]));
    }
