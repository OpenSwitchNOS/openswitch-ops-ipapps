...
| Responsibility : Display the source interface details
| Parameters : type : Specify the type of the protocol
| Return : CMD_SUCCESS
-----------------------------------------------------------------------------*/
static int
show_source_interface_selection(SOURCE_INTERFACE_PROTOCOL type)
{
    ...
    bool is_common_proto_source = false;
    ...
    {
        /* To display the source interface details for the specified protocol */
        is_proto_source = get_protocol_source_local(type,
    ...
    else
    {
        ...
        is_common_proto_source = get_protocol_source_local(ALL_PROTOCOL,
                                DEFAULT_VRF_NAME, &proto_source);
        is_proto_source = get_protocol_source_local(TFTP_PROTOCOL,
                                DEFAULT_VRF_NAME, &proto_source);
        if (is_proto_source)
        {
            vty_out(vty, " %-15s %-46s ", protocol_keys[TFTP_PROTOCOL],
                    proto_source.source);
        }
        else if (is_common_proto_source)
        {
            vty_out(vty, " %-15s %-46s ", protocol_keys[TFTP_PROTOCOL],
                    proto_source.source);
        }
        ...
        {
            vty_out(vty, " %-15s %-6s ", protocol_keys[TFTP_PROTOCOL], "");
        }
    }
                       ...
                       const char *key, bool add)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                vty_out(vty, "Common source interface is not configured.%s",
                        VTY_NEWLINE);
                return;
            }
            else if (!strcmp(key, VRF_SOURCE_IP_MAP_TFTP))
            {
                vty_out(vty, "TFTP Source interface is not configured.%s",
                        VTY_NEWLINE);
                return;
            }
        }
