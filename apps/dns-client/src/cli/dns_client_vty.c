                      ...
                      bool set, char *value, DNS_CLIENT_FIELDS field)
{
    ...
    else
    {
        ...
        {
            if (strncmp (value, dnsClientArray[i], strlen(value) != 0))
...
{
    size_t v4hostcount = 0, v6hostcount = 0, total = 0;
    struct smap smapv4, smapv6;
    struct smap_node *nodev4, *nodev6;
    char *curhostname = NULL, *curhostnamev6 = NULL;

    smap_clone(&smapv4, &dns_client_row->host_v4_address_mapping);
    smap_clone(&smapv6, &dns_client_row->host_v6_address_mapping);

    /* fetch the current count */
    v4hostcount = smap_count(&smapv4);
    v6hostcount = smap_count(&smapv6);

    /* check if there are duplicate entries in the columns */
    SMAP_FOR_EACH (nodev4, &smapv4)
    {
        curhostname = xstrdup(nodev4->key);
        ...
        {
            SMAP_FOR_EACH (nodev6, &smapv6)
            {
                curhostnamev6 = xstrdup(nodev6->key);
                if (!strncmp(curhostname, curhostnamev6,
                    strlen(curhostname)))
                {
                    v6hostcount--;
                }
                else if (!strncmp(curhostnamev6, hostname,
                         strlen(hostname)))
                {
                    smap_destroy(&smapv4);
                    smap_destroy(&smapv6);
...
dnsclient_host_config (bool set, const char *argv[])
{
    ...
    else
    {
        ...
        {
            curHostName = xstrdup(node->key);
            if(!strncmp(curHostName, hostname, strlen(hostname)))
            {
                curHostIP = xstrdup(node->value);
                hostFound = true;
                break;
            }
            free(curHostName);
...
show_dns_client(void)
{
    ...
    {
        vty_out(vty, "DNS Client Mode%s: Disabled %s", "", VTY_NEWLINE);
    }
    else
    {
        if (!strncmp(dns_client_status,
            ...
            vty_out(vty, "DNS Client Mode%s: Enabled %s", "", VTY_NEWLINE);
        }
    }
    ...
    OVSREC_DNS_CLIENT_FOR_EACH (dns_client_row, idl)
    {
        ...
        SMAP_FOR_EACH(node, &smap)
        {
            hostname = xstrdup(node->key);
            hostIP = xstrdup(node->value);
