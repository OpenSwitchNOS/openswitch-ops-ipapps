        ...
        HOSTNAME_STR )
{
    ...
    {
        VLOG_ERR("ERROR :This user has no authorisation to "
                 "execute this command\n");
        vty_out(vty, "ERROR :This user has no authorisation to "
                     "execute this command%s", VTY_NEWLINE);
        return CMD_SUCCESS;
    }
...
| Responsibility : Initialise sftp details.
| Parameters     :
|      idl       : Pointer to idl structure
-----------------------------------------------------------------------------*/
static void
sftp_ovsdb_init(void)
{
...
/* Install sftp related vty commands */
void
cli_pre_init(void)
{
    vtysh_ret_val sftp_retval = e_vtysh_error;

    sftp_ovsdb_init();

    sftp_retval = install_show_run_config_context(e_vtysh_sftp_server_context,
                                     &vtysh_sftp_server_context_clientcallback,
                                     NULL, NULL);
    if(e_vtysh_ok != sftp_retval)
    {
