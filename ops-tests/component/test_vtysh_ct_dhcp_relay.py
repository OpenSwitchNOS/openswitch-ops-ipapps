...
def relay_option_82_with_keep_policy(sw1):
    ...
    out = sw1("show dhcp-relay")
    assert re.search("(Option 82\s*:\s*?enabled)", out) is not None and \
        re.search("(Response [V|v]alidation\s*:\s*?disabled)", out) \
        is not None and re.search("(Option 82 [H|h]andle [P|p]olicy\s*:\s*?keep)",
                                  out) is not None
    return True
...
def relay_option_82_with_drop_policy(sw1):
    ...
    out = sw1("show dhcp-relay")
    assert re.search("(Option 82\s*:\s*?enabled)", out) is not None and \
        re.search("(Option 82 [H|h]andle [P|p]olicy\s*:\s*?drop)", out) is not None
    return True
...
def dhcp_relay_option_82_disable(sw1):
    ...
    assert re.search("(Option 82\s*:\s*?disabled)", out) is not None and \
        re.search("(Response [V|v]alidation\s*:\s*?disabled)", out) is not None\
        and re.search("(Option 82 [H|h]andle [P|p]olicy\s*:\s*?replace)", out) \
        is not None and re.search("(Remote ID\s*:\s*?mac)", out) is not None
    return True
...
@pytest.mark.skipif(True, reason="The following command is not working "
                                 "properly in this test case:"
                                 "interface 50-4"
                                 "got this error:"
                                 "Configuration not allowed as parent "
                                 "interface of 50-4 is not split.")
