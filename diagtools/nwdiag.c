...
int main(int argc, char *argv[])
{
    ...
    {
        snprintf(ns_path, NSPATHSIZE, "/var/run/netns/");
    ...
    else if ((strncmp(PING6, argv[2], strlen(PING6)) == 0))
    {
        len += snprintf(target+len, BUFFSIZE, "%s ", PING6);
    }
    else if ((strncmp(TRACEROUTE, argv[2], strlen(TRACEROUTE)) == 0))
    {
        len += snprintf(target+len, BUFFSIZE, "%s ", TRACEROUTE);
    }
    else if ((strncmp(TRACEROUTE6, argv[2], strlen(TRACEROUTE6)) == 0))
    {
