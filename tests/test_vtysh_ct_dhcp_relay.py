...
def dhcp_relay_enable(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to enable dhcp-relay is failed"
...
def dhcp_relay_disable(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to disable dhcp-relay is failed"
...
def dhcp_relayRunningConfigTest(dut01):
    ...
    devIntReturn = dut01.DeviceInteract(command="no dhcp-relay")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to show dhcp-relay in running config is failed"
...
def show_dhcp_relay(dut01):
    ...
    cmdOut = dut01.cmdVtysh(command="show dhcp-relay")
    assert 'DHCP Relay Agent :enabled' in cmdOut, "Test to show dhcp-relay "
...
def helper_addressConfigurationFor_unicastServerIp(dut01):
    ...
    "for unicast server IP is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.10.1' in cmdOut, "Test to set helper-address "
    "configuration for unicast server IP is failed"
...
def helper_addressConfigurationOn_splitInterface(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set helper-address configuration "
    "on split interface is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.80.1' in cmdOut, "Test to set helper-address "
    "configuration on split interface is failed"
...
def helper_addressConfigurationOn_subInterface(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set helper-address configuration "
    "on sub interface is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.90.1' in cmdOut, "Test to set helper-address "
    "configuration on sub interface is failed"
...
def helper_addressUnconfigurationFor_unicastServerIp(dut01):
    ...
    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.10.1' not in cmdOut, "Test to unset helper-address "
    "configuration for unicast server IP is failed"
...
def helper_addressUnconfigFor_unConfigdHelperAddress(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to unset helper-address configuration"
    "for unconfigured helper-address is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert 'No such entries are present.' in cmdOut, "Test to"
    " unset helper-address configuration for unconfigured "
    "helper-address is failed"
...
def helper_addressConfigurationFor_multicastServerIp(dut01):
    ...
    "multicast server IP is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert 'Broadcast, multicast and loopback addresses are not \
allowed.' in cmdOut, "Test to validate helper-address configuration \
    for multicast server IP is failed"
...
def helper_addressConfigurationFor_broadcastServerIp(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to validate helper-address configuration for "
    "broadcast server IP is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert 'Broadcast, multicast and loopback addresses are not \
allowed.' in cmdOut, "Test to validate helper-address configuration \
    for broadcast server IP is failed"
...
def helper_addressConfigurationFor_loopbackServerIp(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to validate helper-address configuration for "
    "loopback server IP is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert 'Broadcast, multicast and loopback addresses are not \
allowed.' in cmdOut, "Test to validate helper-address configuration \
    for loopback server IP is failed"
...
def helper_addressConfigurationFor_invalidServerIp(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode != 0, "Test to validate helper-address configuration for "
    "invalid server IP is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert '% Unknown command.' in cmdOut, "Test to validate helper-address \
    configuration for invalid server IP is failed"
...
def helper_addressRecordDuplication_OnsingleInterface(dut01):
    ...
    "on single interface is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert 'This entry already exists.' in cmdOut, "Test to check "
    " helper-address record duplicatio on single interface is failed"
...
def multipleHelper_addressConfigFor_singleInterface(dut01):
    ...
    192.168.10.4")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set multiple helper-address configuration "
    "for single interface is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.10.2' and '192.168.10.3' and '192.168.10.4' in cmdOut, \
        "Test to set multiple helper-address configuration for \
        single interface is failed"
...
def maximumHelper_addressConfiguration_perInterface(dut01):
    ...
    "per interface is failed"
    dut01.DeviceInteract(command="exit")

    cmdOut = devIntReturn.get('buffer')
    assert '192.168.100.19: Entry not allowed as maximum entries per interface exceeded.' in cmdOut, "Test to \
    validate helper-address configuration for invalid server IP is failed"
...
def singleHelper_addressConfigOn_differentInterfaces(dut01):
    ...
    192.168.10.12")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set single helper-address configuration on "
    "different interfaces is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False
    ...
    "on different interfaces is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.10.12' and '192.168.10.12' and '192.168.10.12' in cmdOut, \
...
def show_helper_address_status(dut01):
    ...
    192.168.30.2")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to show helper-address configuration is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address")
    assert '192.168.30.2' in cmdOut, "Test to show helper-address "
    "configuration is failed"
    return True
...
def show_helper_address_statusOnSpecifiedInterface(dut01):
    ...
    " on specified interface is failed"
    dut01.DeviceInteract(command="exit")
    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip helper-address interface 11")
    assert '192.168.50.2' in cmdOut, "Test to show helper-address "
    "configuration on specified interface is failed"
    return True
...
def helper_address_runningConfigTest(dut01):
    ...
    cmdOut = dut01.cmdVtysh(command="show running-config")
    assert 'ip helper-address 192.168.90.11' in cmdOut, "Test to show "
    "helper-address configuration in running config is failed"
    return True
...
def helper_address_interface_runningConfigTest(dut01):
    ...
    assert 'ip helper-address 192.168.90.77' and 'ip helper-address \
192.168.90.78' in cmdOut, "Test to show helper-address configuration \
    in running config is failed"
...
running configuration - passed")
            ...
            LogOutput('error', "Show DHCP-Relay running \
...
configuration for multicast server IP - passed")
        else:
            LogOutput('error', "Test to validate helper-address \
configuration for multicast server IP - failed")
    ...
    def test_helper_addressConfigurationFor_broadcastServerIp(self):
        ...
        if(retValue):
            LogOutput('info', "Test to validate helper-address configuration \
for broadcast server IP - passed")
        else:
            LogOutput('error', "Test to validate helper-address \
configuration for broadcast server IP - failed")
...
configuration for loopback server IP - passed")
            ...
            LogOutput('error', "Test to validate helper-address \
configuration for loopback server IP - failed")
...
configuration for invalid server IP - failed")
    ...
    def test_helper_addressRecordDuplication_OnsingleInterface(self):
        ...
        if(retValue):
            LogOutput('info', "Test to check helper-address \
record duplication on single interface - passed")
        else:
...
configuration on different interfaces - passed")
        else:
            LogOutput('info', "Test to set single helper-address \
configuration on different interfaces - failed")
...
configuration - failed")
    ...
    def test_show_helper_address_statusOnSpecifiedInterface(self):
        ...
        if(retValue):
            LogOutput('info', "Test to show helper-address \
configuration on specified interface - passed")
        else:
            LogOutput('info', "Test to show helper-address \
configuration on specified interface - failed")
    ...
    def test_helper_address_runningConfigTest(self):
        ...
        if(retValue):
            LogOutput('info', "Test to show helper-address \
configuration in running config - passed")
        else:
            LogOutput('info', "Test to show helper-address \
configuration in running config - failed")
    ...
    def test_helper_address_interface_runningConfigTest(self):
        ...
        if(retValue):
            LogOutput('info', "Test to show helper-address \
configuration in interface running config - passed")
        else:
            LogOutput('info', "Test to show helper-address \
configuration in interface running config - failed")
