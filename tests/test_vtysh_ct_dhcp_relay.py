...
def helper_addressConfigurationFor_unicastServerIp(dut01):
    ...
    "configuration for unicast server IP failed"
...
def helper_addressUnconfigFor_unConfigdHelperAddress(dut01):
    ...
    assert 'Helper-address is not present.' in cmdOut, "Test to"
    " unset helper-address configuration for unconfigured "
    "helper-address failed"
...
def helper_addressRecordDuplication_OnsingleInterface(dut01):
    ...
    " helper-address record duplicatio on single interface failed"
...
def maximumHelper_addressConfiguration_perInterface(dut01):
    ...
    cmdOut = devIntReturn.get('buffer')
    assert '192.168.100.10: Entry not allowed as maximum entries per interface exceeded.' in cmdOut, "Test to \
    validate helper-address configuration for invalid server IP failed"
