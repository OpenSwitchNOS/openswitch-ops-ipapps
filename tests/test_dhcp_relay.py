...
def dhcp_relay_enable(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute dhcp-relay command"

    if (exitContext(dut01) is False):
        return False

    output = dut01.cmd("ovs-appctl -t ops-udpfwd udpfwd/dump")
    assert 'DHCP Relay is enabled' in output, "Test to enable "
    " dhcp-relay failed"
    return True
...
def maximum_helper_address_configuration_per_interface(dut01):
    ...
    retBuffer = dut01.cmd("ovs-appctl -t ops-udpfwd udpfwd/dump interface 3")
    assert '192.168.10.1' in retBuffer \
        ...
        and '192.168.10.8' in retBuffer \
        and '192.168.10.9' in retBuffer \
        and '192.168.10.10' in retBuffer \
        and '192.168.10.11' in retBuffer \
        and '192.168.10.12' in retBuffer \
        and '192.168.10.13' in retBuffer \
        and '192.168.10.14' in retBuffer \
        and '192.168.10.15' in retBuffer \
        and '192.168.10.16' in retBuffer, "maximum helper-address"
...
def same_helper_address_on_multiple_interface(dut01):
    ...
    dut01.DeviceInteract(command="int 7")
    devIntReturn = dut01.DeviceInteract(command="ip helper-address 10.10.10.1")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, " Failed to execute helper-address"
    " configuration command"
    ...
    assert '10.10.10.1' in output \
        and '10.10.10.1' in output, "Test to set same helper-address"
    "configuration failed"
...
def all_interfaces(dut01):
    ...
    dut01.DeviceInteract(command="int 5")
