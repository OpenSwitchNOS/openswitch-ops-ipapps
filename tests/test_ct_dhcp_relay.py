...
def verify_dhcp_relay_statistics(dut01):

    if dhcp_relay_enable(dut01) is False:
        return False
    if helper_address_configuration_per_interface(dut01) is False:
        return False

    retBuffer = dut01.cmd("ovs-appctl -t ops-udpfwd udpfwd/dump interface 1")
    assert 'client request dropped packets = 0' and \
        'client request valid packets = 0' and \
        'server request dropped packets = 0' and \
        'server request valid packets = 0' in retBuffer, \
        "Failed to verify dhcp-relay statistics"

    return True


def verify_dhcp_relay_option82_statistics(dut01):

    if dhcp_relay_enable(dut01) is False:
        return False

    if dhcp_relay_option_82_enable(dut01) is False:
        return False
    if helper_address_configuration_per_interface(dut01) is False:
        return False

    retBuffer = dut01.cmd("ovs-appctl -t ops-udpfwd udpfwd/dump interface 1")
    assert 'client request dropped packets with option 82 = 0' and \
        'client request valid packets with option 82 = 0' and \
        'server request dropped packets with option 82 = 0' and \
        'server request valid packets with option 82 = 0' in retBuffer, \
        "Failed to verify dhcp-relay option 82 statistics"
...
class Test_dhcp_relay_configuration:
    ...
    def test_verify_dhcp_relay_statistics(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        retValue = verify_dhcp_relay_statistics(dut01Obj)
        if(retValue):
            LogOutput('info', "Test to verify dhcp-relay"
                      " statistics - passed")
        else:
            LogOutput('info', "Test to verify dhcp-relay"
                      " statistics - failed")

    def test_verify_dhcp_relay_option82_statistics(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        retValue = verify_dhcp_relay_option82_statistics(dut01Obj)
        if(retValue):
            LogOutput('info', "Test to verify dhcp-relay"
                      " option 82 statistics - passed")
        else:
            LogOutput('info', "Test to verify dhcp-relay"
                      " option 82 statistics - failed")