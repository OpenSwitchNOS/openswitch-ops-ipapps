...
### Configure dhcp-relay option 82 policy
### Description
Configure the dhcp-relay option 82 policy on dut01 using the `dhcp-relay option 82 replace` command.
### Test result criteria
#### Test pass criteria
Verify that the dhcp-relay option 82 policy replace is reflected in the `show dhcp-relay` command.
#### Test fail criteria
Verify that the dhcp-relay option 82 policy replace is not reflected in the `show dhcp-relay` command.

### Configure dhcp-relay option 82 remote ID
### Description
Configure the dhcp-relay option 82 policy on dut01 using the `dhcp-relay option 82 replace mac` command.
### Test result criteria
#### Test pass criteria
Verify that the dhcp-relay option 82 remote ID mac is reflected in the `show dhcp-relay` command.
#### Test fail criteria
Verify that the dhcp-relay option 82 remote ID mac is not reflected in the `show dhcp-relay` command.
